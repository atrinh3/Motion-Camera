...
// This method will start the video stream.  Based on the behavior of
// the raspistill method, this should be non-blocking which means
// separate operations should be able to be made to determine if
// movement is still happening simultaneously.
void streamVideo(string &date)
{
    if((pid = fork()) = 0)
    {
        char **cmd;
        cmd = "raspivid -o %s.h264 -t %i", *date, 600 * 1000;
...
// Use this method to upload to something like Dropbox account so 
// that on board memory isn't used to save so many video files.
void uploadVideo(string& date)
{

}


void removeVideo(string *date)
{
    cmd = "rm %s.h264", date;
    execv("/usr/bin/raspivid", cmd);
}
...
char getDate()
{
    ...
    output = strftime(buffer, sizeof(buffer), "%D %H-%M-%S", &timeinfo);
    return output;
}
    ...
    */
    ...
    while(lowPower)
    {
        ...
        if(!lowPower)
        {
            string date;
            date = getDate();
            // need to get date information here to input into the 
            // streamVideo() method for filename purposes
            streamVideo(&date);
            while(!lowPower)
            {
                getImage(options);
                ...
                if(DEBUG)
                {
                    printf("Getting updated picture w/ video running to check for movement.\n");
                    elapsedTime = difftime(timer, startTime);
                    printf("Elapsed time from ref picture = " + elapsedTime + " s.\n");
                }
                getDifference(&reference, &compare);
